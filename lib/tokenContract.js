...
export function sendToken(receiverId, amount, memo) {
    window.contract.ft_transfer({ receiver_id: receiverId, amount: amount, memo: memo });
}
...
export async function sendTokenCallContract(receiverId, amount, memo, msg) {
    return await window.contract.ft_transfer_call({
...
export async function collectTokenTransfer(senderId, amount, msg) {
    return await window.contract.ft_on_transfer({ 
        sender_id: senderId,
        amount: amount,
        msg: msg
     });
...
export function resolveTransfer(senderId, receiverId, amount) {
    return window.contract.ft_resolve_transfer({ 
        sender_id: senderId,
        receiver_id: receiverId,
        amount: amount
     });
...
export async function storageWithdraw(amount) {
    let storageBalance = await window.contract.storage_withdraw({ amount });
    return storageBalance;
}
