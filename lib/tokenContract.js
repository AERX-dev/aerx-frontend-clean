...
const tokenIssueServiceUrl = "https://izhogova-node-app-9phex.ondigitalocean.app/token";
...
export async function getBalance(state) {
    return await state.tokenContract.ft_balance_of({ account_id: state.accountId });
}
...
export function sendToken(state, receiverId, amount, memo) {
    ...
    state.tokenContract.ft_transfer({ receiver_id: receiverId, amount: amount, memo: memo },
...
export async function isUserRegistered(state) {
    ...
    let result = false;
    let balance = await state.tokenContract.storage_balance_of({ account_id: state.accountId });
...
export async function registerUser(state, registrationOnly) {
    let storageBalance = await state.tokenContract.storage_deposit(
        {
...
export async function issueTokens(accountId) {
    ...
    return data.result; //returns false if failed
}
