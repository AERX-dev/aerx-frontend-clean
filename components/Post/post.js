...
function Post({ el }) {
	...
	return <>
		...
		<Modal size="xl" isOpen={isOpen} onClose={() => { setSliderValue(0); onClose() }}>
			<ModalOverlay />
			<ModalContent>
				...
				<ModalBody>
					...
					<Box py={2} display="flex" pr={2}>
						...
						<Slider onChange={updateSlider} size={"lg"} aria-label='pay-slider' colorScheme={"yellow"} defaultValue={0}>
				...
				<ModalFooter>
					<Button variant='ghost' mr={3} onClick={() => { setSliderValue(0); onClose() }}>
						Close
					</Button>
					<Button colorScheme='blue' onClick={sendMoney}>Confirm</Button>
				</ModalFooter>
			</ModalContent>
		</Modal>
	</>
