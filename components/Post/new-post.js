...
function NewPost () {
	...
	async  function createPost () {
		...
		const { data, error } = await supabase
			.from('posts')
			.insert([
				{ profileId: profileState.profile.id, body, walletId: profileState.profile.walletId }
			]);
	...
	return (
		...
		<Modal isOpen={isOpen} onClose={onClose} size="xl">
			...
			<ModalContent mx={5} rounded="xl">
				...
				<ModalFooter>
				<Box fontSize="sm" color="red.500">{ postError }</Box>
					<Button variant='ghost' mr={3} onClick={onClose}>
						Close
					</Button>
					<Button colorScheme='blue' onClick={ createPost }>Confirm</Button>
				</ModalFooter>
			</ModalContent>
		</Modal>
	</>
